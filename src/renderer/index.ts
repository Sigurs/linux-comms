...
function setupEventListeners() {
	...
	form.addEventListener('submit', async (e) => {
		...
		const extraFields = document.querySelectorAll<HTMLInputElement>(
			'#provider-extra-fields input',
		);
		...
		try {
			const profile = await window.electronAPI.addProfile(
				providerId,
				name,
				config,
			);
	...
	window.electronAPI.onProfileUpdated(
		({ profiles: newProfiles, providers }) => {
	...
	document
		.getElementById('btn-dismiss-warning')
	...
	document.getElementById('btn-about-github')?.addEventListener('click', () => {
		void window.electronAPI.openExternal(
			'https://github.com/Sigurs/linux-comms',
		);
	});
	...
	webviewContainer.addEventListener(
		...
		(e: WheelEvent) => {
			...
			const newZoom = Math.max(
				ZOOM_MIN,
				Math.min(ZOOM_MAX, currentZoom + delta),
			);
...
async function openAboutModal() {
	...
	const licensesBox = document.getElementById(
		'about-licenses',
		...
		try {
			...
			licenseText = entries
				.map((pkg) => {
					...
					const body =
						pkg.licenseText ??
						`See https://spdx.org/licenses/${pkg.license.replace(/\s+/g, '-')}.html`;
					return `${header}\n${divider}\n${body}`;
				})
				.join('\n\n\n');
		} catch {
...
function showIconPicker(profile: Profile): Promise<ProfileIcon | null> {
	return new Promise((resolve) => {
		...
		const customUrlInput = document.getElementById(
			'icon-picker-custom-url',
		) as HTMLInputElement;
		const customPreviewWrap = document.getElementById(
			'icon-picker-custom-preview-wrap',
		)!;
		...
		function selectCell(cell: HTMLElement, icon: ProfileIcon) {
			overlay
				.querySelectorAll<HTMLElement>('.icon-picker-cell')
				.forEach((c) => {
		...
		searchInput.addEventListener('input', () =>
			buildLibraryGrid(searchInput.value),
		);
		...
		customUrlInput.addEventListener('input', () =>
			tryLoadCustomUrl(customUrlInput.value.trim()),
		);
		customPreview.addEventListener('click', () => {
			if (customUrlInput.value.trim())
				selectCustomUrl(customUrlInput.value.trim());
		});
