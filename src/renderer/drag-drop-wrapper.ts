export class DragDropWrapper {
  ...
  private handleDragMove = (e: MouseEvent): void => {
    ...
    const draggingEntry = document.querySelector('.profile-entry.dragging') as HTMLElement;
    if (!draggingEntry) return;

    // Simple drag implementation - in a real app, use a proper drag library
    const entries = Array.from(this.container.querySelectorAll('.profile-entry:not(.dragging)'));
  ...
  private showCountdown(_entry: HTMLElement): void {
