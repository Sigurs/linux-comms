...
function validateProvider(p: CommunicationProvider): boolean {
  for (const field of REQUIRED_FIELDS) {
    if (!p[field]) {
      console.error(`[providers] Provider "${p.id ?? '?'}" is missing required field: ${field}`);
      return false;
    }
  }
  if (!/^[a-z0-9-]+$/.test(p.id)) {
...
const rawProviders: CommunicationProvider[] = [teamsProvider, rocketchatProvider];

export const providers: CommunicationProvider[] = rawProviders.filter(validateProvider);
