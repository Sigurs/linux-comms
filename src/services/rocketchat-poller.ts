...
import {
	type RocketChatUnreadCounts,
	RocketChatUnreadService,
} from './rocketchat-api';
...
export class RocketChatPoller {
	...
	private scheduleNextPoll(profileId: string, delayMs: number = 5000): void {
		...
		if (poller.consecutiveFailures >= 3) {
			console.warn(
				`RocketChat poller ${profileId}: circuit breaker triggered, stopping polls`,
			);
			return;
		}
	...
	private async pollNow(profileId: string): Promise<void> {
		...
		if (
			poller.cachedCounts &&
			poller.cacheTimestamp &&
			now - poller.cacheTimestamp < 30000
		) {
		...
		try {
			...
			if (poller.cachedCounts) {
				console.log(
					`RocketChat poller ${profileId}: using cached data as fallback`,
				);
				this.onBadgeUpdate(profileId, poller.cachedCounts.total);
			}
			...
			const retryDelay = Math.min(
				30000,
				1000 * 2 ** poller.consecutiveFailures,
			);
			console.log(
				`RocketChat poller ${profileId}: retrying in ${retryDelay}ms`,
			);
	 ...
	 */
	cleanup(): void {
		this.pollers.forEach((poller, _profileId) => {
			if (poller.intervalId) {
				clearTimeout(poller.intervalId);
			}
		});
