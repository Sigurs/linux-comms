...
export class RocketChatUnreadService {
  ...
  private normalizeServerUrl(url: string): string {
    ...
    try {
      ...
      console.error(`Invalid RocketChat URL for profile ${this.profileId}:`, url);
  ...
  private async makeApiRequest<T>(
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error(`RocketChat API error: ${response.status} ${response.statusText}`);
      }

      return (await response.json()) as T;
    } catch (error) {
      console.error(`RocketChat API request failed for ${this.profileId}:`, error);
      throw error;
    }
  }
  ...
  async getUnreadCounts(): Promise<RocketChatUnreadCounts> {
    ...
    try {
      ...
      console.error(`Failed to get unread counts for profile ${this.profileId}:`, error);
