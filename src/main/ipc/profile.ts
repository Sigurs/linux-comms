...
export function registerProfileIpc(): void {
	...
	ipcMain.handle(
		IPC.PROFILE_REMOVE,
		(_event, profileId: string, partition: string) => {
	...
	ipcMain.handle(
		IPC.PROFILE_RENAME,
		(_event, profileId: string, newName: string) => {
	...
	ipcMain.handle(
		IPC.PROFILE_UPDATE_ZOOM,
		(_event, profileId: string, zoomLevel: number) => {
	...
	ipcMain.handle(
		IPC.PROFILE_UPDATE_ICON,
		(_event, profileId: string, icon: ProfileIcon | undefined) => {
