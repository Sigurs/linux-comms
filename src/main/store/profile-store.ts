...
import type { Profile, ProfileIcon, ProfilesData } from '../../shared/types';
...
export function updateZoomLevel(
	...
	if (clampedZoom !== zoomLevel) {
		console.warn(
			`[profile-store] Zoom level ${zoomLevel} clamped to ${clampedZoom}`,
		);
	}
...
const _debounceTimer: NodeJS.Timeout | null = null;
...
export function updateProfileOrder(profileIds: string[]): boolean {
	...
	if (invalidIds.length > 0) {
		console.error(
			`[profile-store] Invalid profile IDs in order update: ${invalidIds.join(', ')}`,
		);
		return false;
	}
